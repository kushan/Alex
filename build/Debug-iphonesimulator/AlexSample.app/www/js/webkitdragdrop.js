...
var webkit_click = function(r, ip, top, left)
{
	this.initialize = function(root, instance_props, top, left)
	{
		var default_props = {onClick : webkit_tools.empty};
		try {
		this.root = webkit_tools.$(root);
		this.root.style.top = top || this.root.style.top;
		this.root.style.left = left || this.root.style.left;
		this.p = webkit_tools.extend(default_props, instance_props || {});
		this.bindEvents();
		}catch(ex) {
			showError('webkit_click ex:' + ex);
		}
	}
